...
| ⚡ **[Live Chat, Human Handover & WebSocket API](live_chat_and_handover_websocket_api.md)** | Sub-50ms bidirectional WebSocket hub, human takeover & handback, multi-session operator triage, staff whisper notes, and audio chime alerts. | `/api/v1/ws/sessions/{key}`, `/api/v1/ws/sessions/global`, `/takeover`, `/handback` |
...
| 📱 **[WhatsApp Integration (OpenWA)](whatsapp_integration_api.md)** | Staff notification relay + guest-facing WhatsApp concierge via a self-hosted OpenWA gateway — connect flow, inbound webhook mechanics (HMAC, rate limiting, kind filtering), the guest round trip (identity, agent invocation, image composition), and human handover integration. | `integrations/whatsapp/connect`, `integrations/whatsapp/webhook/{sessionId}`, `integrations/messaging/send`, `FindOrCreateGuestByPhone` |
| 📐 **[ADR: Connecting Hotel WhatsApp Numbers via OpenWA](adr_whatsapp_openwa_integration.md)** | The architecture decision record behind the WhatsApp integration above — options considered, decision drivers, verified technical corrections, risk register (including two real production incidents), and the shipped/remaining rollout split. | Options A–D, inbound architecture, ad-attribution honesty, risk register |
